...
| `AH_RELEASE_GPU_ON_RUN_END=0` | Keep warm workers (e.g. `$image2image`, `$image2video`) after a run finishes |

When a `.ah` run finishes (success, error, or cancel), Anthill calls `release_gpu_resources()`:
stops warm workers for `$image2image` / `$image2video` and any still-running external subprocesses, then
