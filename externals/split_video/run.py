"""$split_video — split videos on frame-hash scene changes."""
...
def _emulate_enabled() -> bool:
    return os.environ.get("AH_EMULATE_SPLIT_VIDEO", "").lower() in (
        "1",
        "true",
        "yes",
    )
...
def _int_arg(args: dict[str, str], key: str, default: int, *, min_value: int = 1) -> int:
    ...
    value = int(raw)
    if value < min_value:
        raise ValueError(f"$split_video: {key}= must be >= {min_value}, got {value}")
    return value
...
def _help() -> str:
    return (
        "$split_video needs videos[] in the input bundle.\n"
        "  tools\\setup_external_venvs.ps1   (or uv sync --extra split_video)\n"
        "  uv run python tools/download_ffmpeg.py\n"
        "  threshold=10  hash_size=8  min_frames=100\n"
        "Test without ffmpeg/models: AH_EMULATE_SPLIT_VIDEO=1"
    )
...
def run(ctx: ExternalContext, inp: ExternalInput) -> ArrayBundle:
    ...
    if _emulate_enabled():
        for index, link in enumerate(video_links):
            ...
            content = (
                f"[emulated $split_video threshold={threshold} "
                f"hash_size={hash_size} min_frames={min_frames}] "
                f"{src_path.name}\n"
            )
    ...
    from externals.split_video.split import _require_deps, detect_fragments
    ...
    for link in video_links:
        if ctx.cancel_event is not None and ctx.cancel_event.is_set():
            ...
            raise RuntimeCancelled("$split_video cancelled")
        ...
        if not fragments:
            print(f"$split_video: {src_path.name} — no frames, skipped", flush=True)
            continue

        print(
            f"$split_video: {src_path.name} — {len(fragments)} fragment(s), "
