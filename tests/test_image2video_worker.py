"""Tests for $image2video warm worker."""
...
from externals.image2video.worker_client import worker_enabled
...
class TestImage2VideoWorker(unittest.TestCase):
    def test_worker_enabled_by_default(self) -> None:
        os.environ.pop("AH_IMAGE2VIDEO_WORKER", None)
        self.assertTrue(worker_enabled())
        os.environ["AH_IMAGE2VIDEO_WORKER"] = "0"
        self.assertFalse(worker_enabled())
        os.environ.pop("AH_IMAGE2VIDEO_WORKER", None)

    def test_emulate_via_worker(self) -> None:
        os.environ["AH_EMULATE_IMAGE2VIDEO"] = "1"
        os.environ["AH_IMAGE2VIDEO_WORKER"] = "1"
        try:
            from externals.image2video.worker_client import run_via_worker, terminate_worker
