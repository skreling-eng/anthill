...
class Parameters:
    ...
    DEFAULT_HEADER_FONT_COLOR = 'white'
    ...
    def __init__(self, path: Path, recursive: bool, width: int, columns: int, rows: int,
                 ...
                 skip_seconds: float, suffix: str, subdir: str, jpeg_quality: int, override_existing: bool, output_directory: Path,
...
class VideoThumbnailer:
    ...
    def __init_parameters(self):
        ...
        if config_file.is_file():
            ...
            if ConfigFile.CONFIG_SECTION_FILE in config:
                ...
                if 'subdir' in file_options:
                    self.parameters.subdir = config.get(ConfigFile.CONFIG_SECTION_FILE, 'subdir')
        ...
        if args.subdir is not None:
            self.parameters.subdir = args.subdir
    ...
    def create_and_save_preview_thumbnails_for(self, file_path: Path) -> None:
        ...
        if self.parameters.subdir is None:
            self.parameters.subdir = ''
        image_path = None
        if self.parameters.output_directory is None:
            image_dir = os.path.dirname(file_path)
            if image_dir == '':
                image_dir = '.'
            image_dir += '/' + self.parameters.subdir + '/'
            Path(image_dir).mkdir(parents=True, exist_ok=True)
            print(image_dir)
            image_path = Path('{}{}{}.jpg'.format(image_dir, file_path.name, self.parameters.suffix))
            print(image_path)
        else:
