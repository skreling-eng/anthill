...
class VAE:
    ...
    def decode(self, samples_in, vae_options={}):
        ...
        do_tile = False
        try:
            from externals.comfy_inprocess.vae_tiling import force_tiled_vae

            do_tile = force_tiled_vae()
        except ImportError:
            pass
        force_full = self.disable_offload
        if do_tile:
            logging.info("Using tiled VAE decoding (WAN_I2V_TILED_VAE / tiled_vae=1)")
            try:
                from externals.comfy_inprocess.vae_tiling import tiled_vae_force_full_load

                force_full = force_full or tiled_vae_force_full_load()
            except ImportError:
                pass
        ...
        if do_tile:
            ...
            model_management.load_models_gpu(
                [self.patcher], memory_required=memory_used, force_full_load=force_full
            )
    ...
    def encode(self, pixel_samples):
        ...
        force_full = self.disable_offload
        if do_tile:
            logging.info("Using tiled VAE encoding (WAN_I2V_TILED_VAE / tiled_vae=1)")
            try:
                from externals.comfy_inprocess.vae_tiling import tiled_vae_force_full_load
